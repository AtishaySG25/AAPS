...
p = ggplot(data = ?) +
  geom_histogram(aes(x = ?, y = ?), breaks = seq(mean(hData$rent)-4*sd(hData$rent), mean(?)+4*sd(?), by = 25000), color = 'black', fill = 'blue') +
...
hData['logrent'] = ?
p = ggplot(data = ?) +
  geom_histogram(aes(x = ?, y = ?), breaks = seq(mean(?)-4*sd(?), mean(?)+4*sd(?), by = 0.5), color = 'black', fill = 'blue') +
  labs(x = 'Rent', y = 'Frequency')  +
   theme(axis.text = element_text(size = 8),
   axis.text.x = element_text(size = 10),
   axis.text.y = element_text(size = 10),
